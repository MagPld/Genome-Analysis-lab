...
if [ -e ./genome_assembly.ms22.histogram -a -e ./genome_assembly.ms22.dump -a -e ./genome_assembly.ms22.ignore.gz ] ; then
  exit 0
fi
...
if [ -e genome_assembly.ms22.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e genome_assembly.ms22.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e genome_assembly.ms22.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'genome_assembly.ms22.histogram', 'genome_assembly.ms22.dump' and 'genome_assembly.ms22.ignore.gz' exist in 'unitigging/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./genome_assembly.ms22/merylIndex ] ; then
  /sw/bioinfo/canu/2.2/rackham/bin/meryl threads=2 memory=2 \
    greater-than 1 \
      output genome_assembly.ms22.WORKING \
      union-sum  \
        ./genome_assembly.01.meryl \
  && \
  mv -f ./genome_assembly.ms22.WORKING ./genome_assembly.ms22
...
if [ ! -e ./genome_assembly.ms22.histogram ] ; then
  /sw/bioinfo/canu/2.2/rackham/bin/meryl threads=1 memory=1 \
    statistics ./genome_assembly.ms22 \
  > ./genome_assembly.ms22.histogram.WORKING \
  && \
  mv ./genome_assembly.ms22.histogram.WORKING ./genome_assembly.ms22.histogram
fi
...
if [ ! -e ./genome_assembly.ms22.dump ] ; then
  /sw/bioinfo/canu/2.2/rackham/bin/meryl threads=2 memory=2 \
    print ./genome_assembly.ms22.##.dump \
      at-least distinct=0.9999 \
        ./genome_assembly.ms22

  cat ./genome_assembly.ms22.??.dump > ./genome_assembly.ms22.dump
  rm -f ./genome_assembly.ms22.??.dump
fi
...
if [ ! -e ./genome_assembly.ms22.ignore.gz ] ; then
  numKmers=`wc -l < ./genome_assembly.ms22.dump`
  totKmers=`/sw/bioinfo/canu/2.2/rackham/bin/meryl statistics ./genome_assembly.ms22 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./genome_assembly.ms22.dump | gzip -1c > ./genome_assembly.ms22.ignore.gz
fi
