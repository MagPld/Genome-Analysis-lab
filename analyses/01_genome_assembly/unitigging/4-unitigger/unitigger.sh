...
if [ -e genome_assembly.ctgStore/seqDB.v001.tig ]; then
  exists=true
else
  exists=false
fi
...
if [ -e ./genome_assembly.ctgStore ] ; then
  mv ./genome_assembly.ctgStore ../genome_assembly.ctgStore
fi
...
#
if [ -e ../genome_assembly.ctgStore -a \
   ! -e ../genome_assembly.ctgStore/seqDB.v001.sizes.txt ] ; then
  $bin/tgStoreDump \
    -S ../../genome_assembly.seqStore \
    -T ../genome_assembly.ctgStore 1 \
    -sizes -s 3000000 \
   > ../genome_assembly.ctgStore/seqDB.v001.sizes.txt
fi

if [ -e ../genome_assembly.ctgStore ] ; then
  cd ../genome_assembly.ctgStore
  cd -
fi
