...
if [ -e ./genome_assembly.$jobid.meryl/merylIndex ] ; then
...
if [ -e genome_assembly.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/sw/bioinfo/canu/2.2/rackham/bin/meryl k=16 threads=4 memory=3 \
  count \
    segment=$jobid/01 ../../genome_assembly.seqStore \
    output ./genome_assembly.$jobid.meryl.WORKING \
&& \
mv -f ./genome_assembly.$jobid.meryl.WORKING ./genome_assembly.$jobid.meryl
